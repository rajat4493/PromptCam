...
**Invariants:** no recording is silently deleted; no incomplete file is
presented as valid (`RecordingOutcome.isPlayable` is the sole playback gate, and
`playbackURL` additionally requires the file to exist); every terminal outcome
is persisted, so a lost interview leaves a visible trace.
...
| `PROMPTCAM_DUO` | **off** | `SWIFT_ACTIVE_COMPILATION_CONDITIONS` |
...
- `DEVELOPMENT_TEAM` is intentionally **blank**. A committed team identifier
  breaks every other developer's build. Set it in Xcode, or locally in
  `project.yml` without committing.
- Bundle identifier is `com.example.promptcam.PromptCam` — a placeholder that
  must be changed before any device build.
...
- [ ] `DEVELOPMENT_TEAM` set locally
- [ ] Bundle identifier changed from `com.example.promptcam.*`
- [ ] Version and build number set (currently 0.1.0 / 1)
- [ ] App icon and launch screen (neither is in the repository yet)
